...
attach_angle = 6
action_size = 2
agent = MultiStepLearningSailor(mdp.size, action_size, n_step,mean,std,history_duration, mdp_step, time_step)

CNN = "multi_step_learning_memory_80_epochs_10"
agent.load(CNN)
...
for time in range(SIMULATION_TIME):
    ...
    # Il faut faire agir les 2 agents initialisés pour simuler les 2 futurs différents

    for tt in range(time + 1, SIMULATION_TIME):
        # Reward for the on policy action then following policy
        next_state_tmp1, reward_tmp1 = mdp_tmp1.transition(policy_action_tmp1, WH)
        policy_action_tmp1 = agent1.actDeterministicallyUnderPolicy(next_state_tmp1)
