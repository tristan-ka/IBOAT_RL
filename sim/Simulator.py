...
class Simulator:
    ...
    :ivar hysteresis.Hysteresis hyst: Memory of the flow state during the simulations.
    ...
    def __init__(self, duration, time_step):
        ...
        self.hyst = hysteresis.Hysteresis()
    ...
    def computeNewValues(self, delta_hdg, WH):
        Hdg_tmp = self.hdg[len(self.hdg) - 3]
        ...
        if saturation == True:
            for jj in range(self.size):
                self.hdg[jj] = Hdg_tmp
                self.vmg[jj] = self.hyst.calculateSpeed(Hdg_tmp + WH[jj] + self.sail_pos) * math.cos(self.hdg_target - self.hdg[jj])
